...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  const getOneTodoQuery = `
        ...
        WHERE id = ${todoId};
    `;
  const getOneTodo = await db.get(getOneTodoQuery);
...
app.post("/todos/", async (request, response) => {
  ...
  const createATodoQuery = `
    ...
    `;
  const createATodo = await db.run(createATodoQuery);
  const todoId = createATodo.lastID;
